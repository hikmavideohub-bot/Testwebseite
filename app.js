...
/* ===========================================================
   Mobile: show discount price on tap (بدون تغيير المنطق الأساسي)
   - افتراضيًا: يظهر السعر قبل الخصم فقط
   - عند الضغط على السعر: يظهر سعر العرض بالأحمر
   =========================================================== */
document.addEventListener('click', (e) => {
  // only for mobile breakpoint
  if (window.innerWidth > 992) return;

  const priceWrapper = e.target.closest('.price-wrapper');
  if (!priceWrapper) return;

  // if no discount element, ignore
  const discountEl = priceWrapper.querySelector('.price-new.discount');
  if (!discountEl) return;

  // toggle
  priceWrapper.classList.toggle('show-discount');
});
